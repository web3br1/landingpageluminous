...
// Import secure crypto utilities
...
export class ABTestingManager {
  ...
  constructor(
    experimentsDir = path.join(process.cwd(), "tmp", "tdd-experiments"),
  ) {
  ...
  private loadActiveExperiments() {
    try {
      const files = fs
        .readdirSync(this.experimentsDir)
        .filter((f) => f.endsWith(".json"))
        .map((f) => path.join(this.experimentsDir, f));

      this.activeExperiments = files
        .map((file) => {
  ...
  private assignVariant(
    ...
    const hash = sha256Hash(userId + experiment.id);
    const index =
      parseInt(hash.substring(0, 8), 16) % experiment.variants.length;
  ...
  private recordObservation(
    ...
    variant.metrics.avgScore =
      (oldAvgScore * (variant.sampleSize - 1) + score) / variant.sampleSize;
    // variant.metrics.avgExecutionTime = (oldAvgTime * (variant.sampleSize - 1) + executionTime) / variant.sampleSize
    variant.metrics.criticalIssues =
      (variant.metrics.criticalIssues * (variant.sampleSize - 1) +
        criticalCount) /
      variant.sampleSize;
    ...
    variant.metrics.successRate =
      (variant.conversions / variant.sampleSize) * 100;
  ...
  private checkCompletion(experiment: Experiment): void {
    const allVariantsReady = experiment.variants.every(
      (v) => v.sampleSize >= experiment.minSampleSize,
    );
  ...
  private calculateResults(experiment: Experiment): ExperimentResults {
    const variants = experiment.variants.sort(
    ...
    const avgOtherSuccess =
      otherVariants.reduce((sum, v) => sum + v.metrics.successRate, 0) /
  ...
  private generateRecommendations(
    ...
    switch (experiment.targetMetric) {
      case "score":
        recommendations.push(
          `Adotar configuração da variante '${winner.name}' para melhorar scores em ${winner.metrics.avgScore.toFixed(1)} pontos`,
        );
        break;

      case "execution_time":
        recommendations.push(
          `Configuração '${winner.name}' reduz tempo de análise em ${winner.metrics.avgExecutionTime.toFixed(0)}ms`,
        );
        break;

      case "critical_issues":
        recommendations.push(
          `Variante '${winner.name}' reduz problemas críticos em ${winner.metrics.criticalIssues.toFixed(1)}`,
        );
        break;
    }
    ...
    if (winner.config.maturityWeights) {
      recommendations.push(
        "Ajustar pesos de maturidade conforme variante vencedora",
      );
    }
  ...
  private generateInsights(
    ...
    insights.push(
      `Melhor variante teve ${diff.toFixed(1)}% mais sucesso que a pior`,
    );
    ...
    if (maturityVariant && maturityVariant.id === best.id) {
      insights.push(
        "Pesos de maturidade influenciaram significativamente o resultado",
      );
    }
    ...
    if (
      cacheVariant &&
      cacheVariant.metrics.avgExecutionTime <
        best.metrics.avgExecutionTime * 0.9
    ) {
      insights.push(
        "Configurações de cache impactaram performance significativamente",
      );
    }
  ...
  static createDefaultExperiments(): Experiment[] {
    return [
...

      {
        ...
        hypothesis:
          "TTL mais agressivo melhora performance sem perder precisão",
   ...
   */
  generateExperimentsReport(): unknown {
    const active = this.activeExperiments.filter((e) => e.status === "active");
    const completed = this.activeExperiments.filter(
