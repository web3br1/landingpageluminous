...
import type { MaturityLevel, ContextualScores, TDDResults, StaticCoverageProxy, SafeTestResult } from '../types.js'
...
export class HybridMetricsCalculator {
   ...
   */
  private hybridizeMetric(baseValue: number, engineValue: number, engineWeight: number): number {
   ...
   */
  generateConfidenceReport(scores: ContextualScores): any {
    const sources = scores.breakdown.reduce((acc, item) => {
    ...
    const confidence = {
      ...
      recommendations: this.generateConfidenceRecommendations(sources, scores.maturity)
    }
   ...
   */
  private generateConfidenceRecommendations(sources: Record<string, number>, maturity: MaturityLevel): string[] {
    const recommendations: string[] = []

    if (sources.classifier && !sources.engine && maturity !== 'M0') {
      recommendations.push('Considere executar subset seguro para métricas mais precisas')
    }

    if (sources.proxy && sources.proxy > 2) {
      recommendations.push('Métricas de proxy são estimativas - valide com execução real quando possível')
    }

    if (sources.hybrid) {
      recommendations.push('Métricas híbridas combinam sinais confiáveis com estimativas')
    }

    if (maturity === 'M0' && sources.engine) {
      recommendations.push('Execução do Engine em M0 é surpreendente - verifique configuração')
    }
