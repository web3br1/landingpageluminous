...
export class AlertsManager {
   ...
   */
  analyzeAndGenerateAlerts(results: TDDResults, safeTestResult?: SafeTestResult): AlertSummary {
    this.alerts = []
  ...
  private checkMaturityAlerts(results: TDDResults): void {
    ...
    if (maturity === 'M0' && criticalCount >= 3) {
      this.addAlert({
        ...
        impact: 'Bloqueia acesso a métricas avançadas e aumenta risco de regressões',
        recommendation: 'Priorizar correção dos problemas críticos identificados antes de adicionar novas funcionalidades',
    ...
    if (maturity !== 'M0' && score < 40) {
      this.addAlert({
        ...
        impact: 'Indicativo de problemas não detectados ou métricas desatualizadas',
        recommendation: 'Revisar configuração de pesos e validar fontes de métricas',
  ...
  private checkQualityAlerts(results: TDDResults): void {
    ...
    if (redTests.length >= 5) {
      this.addAlert({
        ...
        impact: 'Qualidade do código comprometida, alto risco de bugs em produção',
        recommendation: 'Focar em correção dos problemas mais impactantes primeiro',
        evidence: { criticalCount: redTests.length, issues: redTests.slice(0, 3) },
        actionable: true,
    ...
    if (recurringCategories.length > 0) {
      this.addAlert({
        ...
        recommendation: 'Implementar regras de lint ou revisões específicas para essas categorias',
    ...
    if (yellowTests.length >= 10) {
      this.addAlert({
        ...
        recommendation: 'Resolver avisos não críticos ou ajustar regras de qualidade',
  ...
  private checkCoverageAlerts(results: TDDResults): void {
    ...
    if (!hasRealCoverage && maturity !== 'M0') {
      this.addAlert({
        ...
        description: 'Usando apenas proxy de cobertura - métricas reais não disponíveis',
        impact: 'Métricas aproximadas, menor precisão para tomada de decisão',
        recommendation: 'Melhorar maturidade para habilitar métricas reais de cobertura',
  ...
  private checkSafeTestsAlerts(safeTestResult: SafeTestResult): void {
    ...
    if (successRate < 70) {
      this.addAlert({
        ...
        recommendation: 'Revisar critérios de seleção do safe subset ou corrigir testes flaky',
    ...
    if (flakyCount > 0) {
      this.addAlert({
        ...
        recommendation: 'Investigar e corrigir testes flaky ou removê-los do subset',
        evidence: { flakyCount, flakyTests: safeTestResult.reliability.flakyTests },
        actionable: true,
  ...
  private checkTrendAlerts(results: TDDResults): void {
    ...
    if (results.scores.finalScore < 50) {
      this.addAlert({
        ...
        description: 'Score geral apresentando tendência de queda nas últimas execuções',
        impact: 'Qualidade regredindo ao longo do tempo',
        recommendation: 'Revisar commits recentes e identificar causas da queda',
        evidence: { currentScore: results.scores.finalScore, trend: 'declining' },
        actionable: true,
    ...
    if (results.maturity.level === 'M1') {
      this.addAlert({
        ...
        description: 'Projeto permanece em M1 há múltiplas execuções sem progresso',
        impact: 'Limita acesso a métricas mais avançadas',
        recommendation: 'Focar em resolver problemas críticos para avançar para M2',
   ...
   */
  private findRecurringIssues(issues: any[]): string[] {
  ...
  getActiveAlerts(): AlertSummary {
    ...
    const active = this.alerts.filter(alert =>
