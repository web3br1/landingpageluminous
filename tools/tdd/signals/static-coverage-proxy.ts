...
export class StaticCoverageProxy {
  ...
  async calculateProxy(): Promise<IStaticCoverageProxy> {
    ...
    const proxyCoverage = instrumentableLines > 0 ? (estimatedCovered / instrumentableLines) * 100 : 0
  ...
  private async analyzeFile(filePath: string): Promise<FileCoverage> {
    ...
    try {
      ...
      const estimatedCovered = hasTestFile ? Math.floor(instrumentableLines * 0.7) : 0
      const proxyCoverage = instrumentableLines > 0 ? (estimatedCovered / instrumentableLines) * 100 : 0
  ...
  private estimateInstrumentableLines(content: string): number {
    ...
    for (const line of lines) {
      ...
      if (trimmed.includes('function ') || trimmed.includes('=>') || trimmed.includes('const ') && trimmed.includes('=')) {
        instrumentable++
        continue
      }
      ...
      if (trimmed.includes('.') && !trimmed.includes('import ') && !trimmed.includes('from ')) {
        instrumentable++
        continue
      }
    }
  ...
  generateDirectoryReport(proxy: IStaticCoverageProxy): Record<string, any> {
    ...
    Object.keys(byDirectory).forEach(dir => {
      ...
      stats.coverage = stats.instrumentableLines > 0 ?
        (stats.estimatedCovered / stats.instrumentableLines) * 100 : 0
