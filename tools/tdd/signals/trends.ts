...
export class TDDTrends {
   ...
   */
  private extractTopFindings(results: TDDResults): Array<{
    file: string;
   ...
   */
  generateTrendsReport(days = 30): unknown {
    const history = this.loadHistory();
    ...
    recentEntries.forEach((entry) => {
      entry.topFindings.forEach((finding) => {
        if (finding.severity === "RED") {
          failurePatterns[finding.file] =
            (failurePatterns[finding.file] || 0) + 1;
        }
      });
    });
    ...
    // Estatísticas gerais
    const avgScore =
      recentEntries.reduce((sum, entry) => sum + entry.scores.finalScore, 0) /
      recentEntries.length;
    const maturityDistribution = recentEntries.reduce(
      (dist, entry) => {
    ...
    return {
      ...
      riskyFiles: this.calculateRiskyFiles(),
      trends: {
        scoreTrend: this.calculateTrend(
          recentEntries.map((e) => e.scores.finalScore),
        ),
        maturityTrend: this.calculateMaturityTrend(
          recentEntries.map((e) => e.maturity),
        ),
      },
    };
  }
   ...
   */
  private calculateMaturityTrend(
    maturities: string[],
  ): "improving" | "stable" | "declining" {
    if (maturities.length < 3) return "stable";

    const levels = { M0: 0, M1: 1, M2: 2, M3: 3 };
    const recent = maturities
      .slice(-3)
      .map((m) => levels[m as keyof typeof levels] || 0);
    const older = maturities
      .slice(0, -3)
      .map((m) => levels[m as keyof typeof levels] || 0);
  ...
  cleanup(daysToKeep = 90): void {
    try {
      ...
      console.log(
        `Histórico limpo: ${history.length - filtered.length} entradas removidas`,
      );
    } catch (error) {
      console.warn(
        "Error cleaning history:",
        error instanceof Error ? error.message : String(error),
      );
    }
  }
  ...
  getStats(): unknown {
    ...
    const maturityCount = history.reduce(
      (count, entry) => {
    ...
    const avgScore =
      history.reduce((sum, entry) => sum + entry.scores.finalScore, 0) /
      history.length;
