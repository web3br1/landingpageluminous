...
export class StructuredReporter {
  ...
  generateStructuredReport(
    ...
    const report: StructuredReport = {
      ...
      safeTests: safeTestResult
        ? this.buildSafeTestsSection(safeTestResult)
        : undefined,
      trends: trendsData,
    };
  ...
  private buildScoresSection(results: TDDResults): StructuredReport["scores"] {
    ...
    const sources = breakdown.reduce(
      (acc, item) => {
  ...
  private buildIssuesSection(results: TDDResults): StructuredReport["issues"] {
    const critical = (results.classifier?.redTests || []).map(
      (issue, index) => ({
        id: `CRIT-${index + 1}`,
    ...
    const warnings = (results.classifier?.yellowTests || []).map(
      (issue, index) => ({
        id: `WARN-${index + 1}`,
        category: issue.category,
        title: issue.issue,
    ...
    if (results.classifier?.recommendations) {
      results.classifier.recommendations.forEach((rec, index) => {
        recommendations.push({
          id: `REC-${index + 1}`,
          priority: rec.priority.toLowerCase() as unknown,
          category: "testing",
          title: rec.action,
  ...
  private buildActions(results: TDDResults): StructuredReport["actions"] {
    ...
    if (results.maturity.level === "M0") {
      actions.shortTerm.push(
        "Executar análise granular por categoria de testes",
      );
    ...
    actions.longTerm.push(
      "Implementar monitoramento contínuo da saúde dos testes",
    );
  ...
  private buildCacheSection(
    cache: NonNullable<TDDResults["cache"]>,
  ): StructuredReport["cache"] {
    return {
   ...
   */
  private buildSafeTestsSection(
    result: SafeTestResult,
  ): StructuredReport["safeTests"] {
    return {
      executed: true,
      subset: {
      ...
      reliability: {
        ...
        recommendations: [
          result.reliability.successRate < 80
            ? "Revisar testes flaky no subset"
            : "Subset confiável mantido",
        ],
      },
    };
  }
  ...
  private buildPerformanceSection(
    ...
    const status =
      lcp <= budget.lcp && cls <= budget.cls && inp <= budget.inp
        ? "pass"
        : lcp <= budget.lcp * 1.2 &&
            cls <= budget.cls * 1.2 &&
            inp <= budget.inp * 1.2
  ...
  saveStructuredReport(report: StructuredReport, filename?: string): string {
    ...
    const reportPath = path.join(
      this.reportsDir,
      filename || `structured-report-${timestamp}.json`,
    );
  ...
  loadStructuredReport(filename: string): StructuredReport | null {
    try {
      ...
      console.warn(
        "Error loading structured report:",
