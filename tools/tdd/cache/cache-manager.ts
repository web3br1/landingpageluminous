...
export class CacheManager {
  ...
  generateKey(
    ...
    domains: Record<CacheDomain, string>,
  ): CacheKey {
    const environment = this.detectEnvironment();
    const timestamp =
      Math.floor(Date.now() / (10 * 60 * 1000)) * (10 * 60 * 1000); // Arredonda para 10min

    return {
      branch,
      maturityTarget: maturityTarget as unknown,
      domains,
      environment,
      timestamp,
    };
  ...
  stringToKey(keyStr: string): CacheKey | null {
    ...
    if (
      prefix !== "TDD" ||
      !["M0", "M1", "M2", "M3"].includes(maturityTarget)
    ) {
      return null;
    }
  ...
  get(key: CacheKey): { entry: CacheEntry | null; decisions: CacheDecision[] } {
    ...
    Object.entries(key.domains).forEach(([domain, hash]) => {
      const domainCachePath = path.join(
        this.cacheDir,
        `domain-${domain.toLowerCase()}.json`,
      );
      ...
      try {
        if (fs.existsSync(domainCachePath)) {
          const domainCache = JSON.parse(
            fs.readFileSync(domainCachePath, "utf8"),
          );
      ...
      try {
        if (fs.existsSync(cachePath)) {
          ...
          if (age < ttl) {
            ...
            decisions.forEach(
              (d) =>
                (d.reason = `TTL expired (${Math.round(age / 60000)}min > ${Math.round(ttl / 60000)}min)`),
            );
          }
        }
      } catch (error) {
        decisions.forEach(
          (d) =>
            (d.reason = `Load error: ${error instanceof Error ? error.message : String(error)}`),
        );
      }
    }
  ...
  invalidateDomain(domain: CacheDomain) {
    try {
      ...
      console.warn(
        `Cache invalidation error for ${domain}:`,
        error instanceof Error ? error.message : String(error),
      );
    }
  }
