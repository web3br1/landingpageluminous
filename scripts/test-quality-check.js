...
class TestQualityChecker {
  ...
  analyzeTestFile(filePath) {
    ...
    const criticalIssues = [
      ...
      },
      {
        pattern: /fireEvent\./g,
        message:
          "⚠️  Usa fireEvent - prefira userEvent para interações realistas",
        severity: "medium",
      },
      {
      ...
      },
      {
        ...
        message:
          "⚠️  Asserção fraca - apenas verifica presença, não funcionalidade",
        severity: "low",
      },
    ];
  ...
  checkMockQuality(content, fileName) {
    ...
    const mockDeclarations = content.match(
      /jest\.mock\([^,]+,\s*\(\)\s*=>\s*\{/g,
    );
    ...
    if (mockDeclarations) {
      ...
      if (
        hasEmptyMocks &&
        hasEmptyMocks.length > mockDeclarations.length * 0.5
      ) {
        this.addIssue(
          fileName,
          "⚠️  Muitos mocks vazios - pode ocultar bugs reais",
          "medium",
        );
      }
    }
  ...
  checkAssertionQuality(content, fileName) {
    ...
    const assertions = {
      ...
      interaction: (
        content.match(/toHaveBeenCalled|toHaveBeenCalledWith/g) || []
      ).length,
      state: (content.match(/toBeVisible|toBeDisabled|toHaveAttribute/g) || [])
        .length,
    };
    ...
    if (totalAssertions > 0 && assertions.presence / totalAssertions > 0.7) {
      this.addIssue(
        fileName,
        "📊 Muitas asserções de presença - adicionar testes funcionais",
        "low",
      );
    }
  }
  ...
  checkTestIsolation(content, fileName) {
    ...
    if (!hasBeforeEach) {
      this.addIssue(
        fileName,
        "🔄 Falta beforeEach - testes podem não estar isolados",
        "medium",
      );
    }
    ...
    if (!hasAfterEach && content.includes("mount|render")) {
      this.addIssue(
        fileName,
        "🧹 Falta afterEach para cleanup - possível vazamento",
        "low",
      );
    }
  }
  ...
  printReport() {
    ...
    if (this.issues.length > 0) {
      ...
      this.issues.forEach((issue) => {
        ...
        console.log(
          `    📁 ${issue.file}${issue.lines.length ? ` (linhas: ${issue.lines.join(", ")})` : ""}`,
        );
      });
    }
    ...
    if (this.warnings.length > 0) {
      ...
      this.warnings.forEach((warning) => {
        ...
        console.log(
          `    📁 ${warning.file}${warning.lines.length ? ` (linhas: ${warning.lines.join(", ")})` : ""}`,
        );
      });
    }
    ...
    console.log("\n" + "=".repeat(60));
    console.log(
      `📊 RESUMO: ${this.issues.length} problemas críticos, ${this.warnings.length} avisos`,
    );
    console.log("=".repeat(60));
