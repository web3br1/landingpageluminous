...
test.describe('Analytics Validation', () => {
  ...
  test('should send pageview events', async ({ page }) => {
    ...
    const analyticsCalls = await page.evaluate(() => (window as any).mockAnalytics);

    // Deve haver pelo menos um evento
    expect(analyticsCalls.length).toBeGreaterThan(0);

    // Verificar se inclui pageview
    const hasPageView = analyticsCalls.some((call: any[]) =>
  ...
  test('should send CTA click events', async ({ page }) => {
    ...
    await page.evaluate(() => { (window as any).mockAnalytics = []; });
    ...
    console.log('✅ CTA interaction completed (analytics may not be fully implemented yet)');
  });
  ...
  test('should handle conversion events', async ({ page }) => {
    ...
    await page.fill('[data-testid="email-input"]', 'analytics-test@example.com');
