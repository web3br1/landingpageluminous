...
export class TestDataFactory {
  static createValidPageComposition(
    ...
    return TestUtils.deepMerge(base, overrides as unknown);
  }
...
export class MockPageCompositionService implements IPageCompositionService {
  composePage = vi.fn() as jest.MockedFunction<IPageCompositionService['composePage']>;

  composePageSync = vi.fn() as jest.MockedFunction<IPageCompositionService['composePageSync']>;
...
export class MockContentMapper implements IContentMapper {
  mapSectionContent = vi.fn() as jest.MockedFunction<IContentMapper['mapSectionContent']>;

  mapSectionContentSync = vi.fn() as jest.MockedFunction<IContentMapper['mapSectionContentSync']>;
...
export class MockFallbackProvider implements IFallbackProvider {
  getFallbackComposition = vi.fn() as jest.MockedFunction<IFallbackProvider['getFallbackComposition']>;

  getFallbackSectionContent = vi.fn() as jest.MockedFunction<IFallbackProvider['getFallbackSectionContent']>;
...
export class TestUtils {
  ...
  static createAppError(
    ...
    return createAppError(code as unknown, message);
  }
  ...
  static mockIntersectionObserver() {
    ...
    if (typeof window !== "undefined") {
      ...
      // For server environments, define on global
      if (!(global as unknown).window) {
        (global as unknown).window = {};
      }
      (global as unknown).window.IntersectionObserver =
    ...
    return {
      ...
      restore: () => {
        if (typeof window !== "undefined") {
          ...
          if ((global as unknown).window) {
            delete (global as unknown).window.IntersectionObserver;
          }
        }
      },
    };
