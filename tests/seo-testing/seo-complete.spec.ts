...
test.describe("Complete SEO Testing", () => {
  test("should validate structured data (JSON-LD)", async ({ page }) => {
    ...
    if (structuredData.length > 0) {
      console.log(
        `✅ Found ${structuredData.length} JSON-LD structured data objects`,
      );

      // Verificar tipos comuns de schema.org
      const schemaTypes = structuredData
        .map((data) => data["@type"] || data.type)
        .filter(Boolean);
      ...
      const validContexts = structuredData.every(
        (data) =>
  ...
  test("should validate Open Graph meta tags", async ({ page }) => {
    ...
    console.log(
      `✅ OG tags present: ${presentTags.length}/${requiredOGTags.length}`,
    );
  ...
  test("should validate Twitter Card meta tags", async ({ page }) => {
    ...
    console.log(
      `✅ Twitter Card tags present: ${presentTags.length}/${requiredTwitterTags.length}`,
    );
  ...
  test("should validate hreflang tags for internationalization", async ({
    page,
  }) => {
    await page.goto("/");

    const hreflangTags = await page
