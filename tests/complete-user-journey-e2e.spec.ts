...
test.describe("Complete User Journey E2E", () => {
  ...
  test("user journey from landing to trial signup", async ({ page }) => {
    ...
    await expect(page).toHaveTitle(/DataFlow|Luminaris|Sistema de Automação/);
    await expect(page.locator('[data-section="hero"]')).toBeVisible();
    await expect(page.locator('[data-section="benefits"]')).toBeVisible();

    // Step 2: Click primary CTA (Começar Grátis or similar)
    ...
    // Step 3: Should navigate to trial/signup page or open modal
    ...
    if (currentUrl.includes("/trial") || currentUrl.includes("/signup")) {
      // We navigated to signup/trial page
      ...
      const submitButton = page
        .getByRole("button", { name: /criar|começar|enviar/i })
        .first();
      await expect(submitButton).toBeVisible();
      await expect(submitButton).toBeEnabled();
    } else {
      // Modal opened or stayed on same page
      const modal = page
        .locator('[role="dialog"]')
      ...
      if (await modal.isVisible({ timeout: 3000 }).catch(() => false)) {
        // Modal opened - check for form elements
        const modalEmailInput = modal.locator('input[type="email"]').first();
        await expect(modalEmailInput).toBeVisible();

        // Fill basic info
        await modalEmailInput.fill("test@example.com");
