...
test.describe("Advanced Security Testing", () => {
  ...
  test("should prevent SQL injection via API calls", async ({
    page,
    request,
  }) => {
    ...
    for (const payload of sqlPayloads) {
      try {
        ...
        console.log(
          `✅ SQL injection prevented for: ${payload.substring(0, 20)}...`,
        );
      } catch (error) {
  ...
  test("should prevent directory traversal attacks", async ({ page }) => {
    ...
    for (const payload of traversalPayloads) {
      try {
        ...
        console.log(
          `✅ Directory traversal prevented for: ${payload.substring(0, 20)}...`,
        );
      } catch (error) {
        // Request pode falhar, mas isso é esperado
        console.log(
          `✅ Directory traversal blocked for: ${payload.substring(0, 20)}...`,
        );
      }
    }
  });
