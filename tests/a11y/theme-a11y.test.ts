...
describe("Theme A11y Validation", () => {
  describe.each(Object.entries(THEME_REGISTRY))(
    "%s theme",
    (themeId, theme) => {
      it("should pass contrast validation for all text combinations", () => {
        const contrastIssues = validateTokenContrast(theme);
        const failures = contrastIssues.filter(
      ...
      it("should generate valid CSS without errors", () => {
        expect(() => {
      ...
      it("should have responsive typography scales", () => {
        ...
        const h1Min = parseFloat(
          typography.scale.h1.match(/clamp\(([^,]+),/)?.[1] || "0",
        );
        const bodyMin = parseFloat(
          typography.scale.body.match(/clamp\(([^,]+),/)?.[1] || "0",
        );
      ...
      it("should have valid border radius scale", () => {
        ...
        const radii = Object.values(radius).map((r) =>
          parseFloat(r.replace("rem", "").replace("px", "")),
        );
      ...
      it("should have valid features configuration", () => {
        ...
        conflictingPairs.forEach(([feature1, feature2]) => {
          if (features[feature1] && features[feature2]) {
            console.warn(
              `${themeId} has conflicting features: ${feature1} and ${feature2}`,
            );
          }
        });
      });
