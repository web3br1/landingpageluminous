...
describe("PageCompositionService", () => {
  ...
  describe("composePage (async)", () => {
    ...
    it("should handle mapper failures gracefully with fallback", async () => {
      ...
      // Assert
      expect(isOk(result)).toBe(true);
      expect(result.value).toEqual(fallbackComposition);
    ...
    it("should validate composition and use fallback on validation failure", async () => {
      ...
      vi.mock("../composer-validation", () => ({
        validatePageComposition: vi.fn().mockReturnValue({
          success: false,
      ...
      // Assert
      expect(isOk(result)).toBe(true); // Should still succeed with fallback
    ...
    it("should handle exceptions and return fallback composition", async () => {
      ...
      // Assert
      expect(isOk(result)).toBe(true); // Should succeed with fallback
      expect(mockFallback.getFallbackComposition).toHaveBeenCalledWith(
        "landing",
        error,
      );
  ...
  describe("experiment collection", () => {
    ...
    it("should aggregate experiments across multiple sections", async () => {
      ...
      // Assert
      expect(result.value.experiments).toHaveLength(1);
      expect(result.value.experiments[0].sections).toEqual([
        "hero",
        "benefits",
      ]);
    });
  ...
  describe("edge cases", () => {
    ...
    it("should validate page composition structure", async () => {
      ...
      vi.doMock("../../lib/composition/composer-validation", () => ({
      ...
      // Restore mock
      vi.doUnmock("../../lib/composition/composer-validation");
    });
    ...
    it("should handle performance monitoring failures gracefully", async () => {
      ...
      // Assert
      expect(isOk(result)).toBe(true); // Should succeed despite perf failure
    });
    ...
    it("should handle error tracker failures gracefully", async () => {
      ...
      // Assert
      expect(isOk(result)).toBe(true); // Should succeed despite tracking failure
    });
    ...
    it("should handle malformed context object", async () => {
      ...
      // Assert
      expect(isOk(result)).toBe(true); // Should handle invalid context gracefully
    });
