...
vi.mock('@/app/(marketing)/components/ui/section-wrapper', () => ({
  SectionWrapper: ({ children, id, className, ...props }: Record<string, unknown>) => (
...
describe('Features Component', () => {
  ...
  const content: FeaturesContent = {
    ...
    layout: 'zigzag',
    features: [
      { icon: 'Database', title: 'Integração de Dados', description: 'Conecte tudo' },
      { icon: 'Zap', title: 'Tempo Real', description: 'Atualizações instantâneas' },
      { icon: 'Brain', title: 'IA Conversacional', description: 'Pergunte aos dados' }
    ]
  }

  it('deve renderizar título, subtítulo e itens de recursos com estrutura adequada', () => {
    render(<Features id="features" content={content} tracking={{ section: 'features' }} />)
    ...
    expect(titleElement).toHaveStyle({ fontSize: expect.stringMatching(/clamp/) })
  ...
  it('deve aplicar atributos de dados para variante e experimento', () => {
    render(
      <Features
        content={content}
        variant="enterprise"
...
describe('Features Component', () => {
    ...
    features: [
      {
        ...
        description: 'Armazenamento seguro e criptografado para todos os seus dados importantes.'
      },
      {
        icon: 'Zap',
        title: 'Performance Rápida',
        description: 'Tecnologia otimizada para máxima velocidade e eficiência.',
        highlight: 'Premium'
      },
      {
        icon: 'Brain',
        title: 'IA Inteligente',
        description: 'Inteligência artificial avançada para automatizar processos complexos.',
        highlight: 'Novo'
      },
      {
        icon: 'Shield',
      ...
      },
      {
        ...
        description: 'Análises profundas e relatórios personalizáveis para tomada de decisão.'
      },
      {
        icon: 'Users',
        title: 'Colaboração em Tempo Real',
        description: 'Trabalhe em equipe com sincronização instantânea e comunicação integrada.'
      }
    ]
  }
  ...
  describe('Rendering', () => {
    it('renders the features section with correct content', () => {
      ...
      expect(screen.getByText(mockFeaturesContent.subtitle)).toBeInTheDocument()
  ...
  describe('Layout Variants', () => {
    it('renders grid layout correctly', () => {
      ...
      const featureCards = document.querySelectorAll('[data-tracking="feature-card"]')
    ...
    it('renders zigzag layout with alternating positions', () => {
      ...
      const featureRows = document.querySelectorAll('.lg\\:flex-row, .lg\\:flex-row-reverse')
  ...
  describe('Hover Effects', () => {
    ...
    it('applies hover effects to icons', () => {
      ...
      const iconContainers = document.querySelectorAll('.group-hover\\:bg-primary\\/20')
